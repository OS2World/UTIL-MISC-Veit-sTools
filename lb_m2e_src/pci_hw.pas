...
begin
  ...
  write('NT: Open SCM       : ');
  writeln(gwiopm_driver.errorlookup(gwiopm_driver.openscm));

  if gwiopm_driver.install('')<>0 then
  begin
    writeln('NT: Driver Problem detected: attempting to fix...');
    write('   NT: Stop driver   : ');
    writeln(gwiopm_driver.errorlookup(gwiopm_driver.stop));
    write('   NT: Remove driver : ');
    writeln(gwiopm_driver.errorlookup(gwiopm_driver.remove));
    writeln('NT: Driver hopefully fixed, retrying...');
  end;


  write('NT: Install driver : ');
  writeln(gwiopm_driver.errorlookup(gwiopm_driver.install('')));
  write('NT: Start driver   : ');
  writeln(gwiopm_driver.errorlookup(gwiopm_driver.start));
  write('NT: Close SCM      : ');
  writeln(gwiopm_driver.errorlookup(gwiopm_driver.closescm));
  write('NT: Open Driver    : ');
  writeln(gwiopm_driver.errorlookup(gwiopm_driver.DeviceOpen));
...
begin
  ...
  GWIOPM_Driver.IOCTL_IOPMD_ACTIVATE_KIOPM;
  write('NT: Close driver  : ');
  writeln(gwiopm_driver.errorlookup(gwiopm_driver.DeviceClose));
  write('NT: Open SCM      : ');
  writeln(gwiopm_driver.errorlookup(gwiopm_driver.openscm));
  write('NT: Stop driver   : ');
  writeln(gwiopm_driver.errorlookup(gwiopm_driver.stop));
